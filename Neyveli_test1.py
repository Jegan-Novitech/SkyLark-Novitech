...
try:
    ...
    (ID INTEGER PRIMARY KEY AUTOINCREMENT,Detail text,temperature text,Device_Id text,Date_Time text)''')
except:
    pass
conn.commit()
conn.close()
...
class App(threading.Thread):
    def __init__(self, tk_root):
        ...
        t1 = threading.Thread(target=rebooting)
        t1.start()
        t2 = threading.Thread(target=retrieve_db)
        t2.start()
...
#                     print('excibit')
                    ...
                    while k:
                        ...
                        date_time=time_now.strftime("%Y-%m-%d %H:%M:%S.%f")
                        conn1 = sqlite3.connect('/home/pi/share/attendance.db')
                        c1 = conn1.cursor()

                        c1.execute("INSERT INTO temp_attendance (Detail, temperature,Device_Id,Date_Time) VALUES (%s,%s,%s,%s)"% ("'"+name+"'","'"+str(temperature123)+"'","'"+str(device_id)+"'","'"+str(date_time)+"'"))
                        print('inserted')
...
#                         self.test_data="~~~~~"                
    ...
    def display_on(self):
        ...
        self.sl5 = Label(self.middleFrame ,width=26,height=1,font= ('Garamond', 20 , 'bold'), justify=LEFT)
        self.sl5.pack(side=TOP, expand = YES ,fill=BOTH,anchor=CENTER,padx=0,pady=1)
        self.sl5.config(text="", fg='black', bg='#33FFEF')
...
#             img = cv2.rotate(img, cv2.ROTATE_180) 
            ...
            self.CurrentTime = now.strftime("%a %d %b %y  %H:%M:%S")
##            print(self.CurrentTime)
            self.sl5.config(text=self.CurrentTime, fg='black', bg='#33FFEF')
