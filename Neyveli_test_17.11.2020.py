...
##        self.vs = VideoStream(src=0,usePiCamera=False).start()
        ...
        self.vs = cv2.VideoCapture(GSTREAMER_PIPELINE, cv2.CAP_GSTREAMER)
        time.sleep(2.0)
...
#         self.top2.pack(side = BOTTOM,fill=BOTH, expand = YES)
    ...
    #                     print('excibit')
                    if(name=="Unknown"):
                        ...
                        try:
                            conn1 = sqlite3.connect('/home/pi/share/attendance.db')
                            c1 = conn1.cursor()

                            c1.execute("INSERT INTO temp_attendance (Detail, temperature) VALUES (%s,%s)"% ("'"+name+"'","'"+str(temperature123)+"'"))
                            print('inserted')
                            ...
                            if(temperature123>float(temp)):
                                ...
                                GPIO.output(BUZZER,0)
                                self.sl2.config(text=str(temperature123)+" °F", fg='RED', bg='#f09f10')
                                subprocess.Popen("echo 'Temperature High Not Allowed' | festival --tts",shell=True)
    ...
    #                         self.sl1.config(text="Server Connection Error", fg='RED', bg='#333579')
            ...
            cnt=1
            count=0
            self.sl4.config(text="", fg='#000000', bg='#000000')
            self.sl3.config(text="", fg='#000000', bg='#000000')
            self.sl1.config(text="SCANNING MASK...", fg='#000000', bg='#000000')
            self.sl2.config(text="", fg='#000000', bg='#000000')
        self.sl2.after(1000, self.mask_test)      
    def display_on(self):
        self.canvas = Canvas(self.topFrame, width = int(ws), height = int(hs/1.5), bg ='black')
        self.canvas.pack(side=TOP,expand = YES ,fill=BOTH,anchor=NW,  padx=0,pady=0)
        
        self.sl2 = Label(self.middleFrame ,width=26,height=1,font= ('Garamond', 34 , 'bold'), justify=LEFT)
        self.sl2.pack(side=LEFT, expand = YES ,fill=BOTH,anchor=CENTER,padx=0,pady=5)
        self.sl2.config(text="Temperature Scanning...", fg='black', bg='#f09f10')
...
#                 print(self.open)
                ...
                try:
                    self.end=time.time()
                    self.total=self.end-self.open
#                     print(self.total,'sec')
                except:
                    self.end=time.time()
                    self.open =time.time()
                    self.total=self.end-self.open
#                     print(self.total,'sec')
            if(self.total>10):
                ...
                self.screensave=1
                img=cv2.imread('coitor.png')
                img=cv2.resize(img, (int(ws), int(hs/1.5)))
                #img = cv2.flip(img, 1)
#                 frame = cv2.addWeighted(src1=img,alpha=1,src2=img,beta=0.2, gamma = 0)
...
#                 blended1 = cv2.addWeighted(src1=img,alpha=1,src2=img1,beta=0.9, gamma = 0)
                ...
                self.screensave=0
                print('screensave',self.screensave)
                img=self.vs.read()
                img=cv2.resize(img, (int(ws), int(hs/1.5)))
                img = cv2.flip(img, 1)
