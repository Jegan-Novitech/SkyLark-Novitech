...
#     calib_temp=round((36.0+(36-38)*(max(tpn))/(33.1-36.1))/1.57,1)
#     #print(calib_temp)
...
#         self.vs = VideoStream(src=0,usePiCamera=False).start()
    ...
    def run(self):
        ...
        self.detail1frame = Frame(self.logo2frame, background ='#ffffff')
        self.detail1frame.pack(side = LEFT, expand = YES,fill=BOTH)
        self.detail2frame = Frame(self.logo2frame, background ='#ffffff')
...
# abs
    ...
    #                         print('entered')
                    ...
                    self.sl1.config(text="SCANING MASK...", fg='black', bg='#D5DBDB')
            except:
                  pass
#                 self.sl2.config(text="          ", fg='black', bg='#D5DBDB')
        elif(check==1):
            print('entered')
            self.sl2.config(text="Temperature Scanning", fg='#FFFFFF', bg='#D5DBDB')
            self.sl3.config(text="Face Scanning...", fg='#FFFFFF', bg='#D5DBDB')
            ...
            if(name=='None'):
                    ...
                    try:
                        ...
                        self.sl3.config(text="Your ID="+name, fg='#ffffff', bg='green')
                        self.test_data=udplib.Attend_send(name+"~"+str(temperature123),host_ip=host_ip,port_no=port_no,bufferSize = 1024)
                        self.sl1.config(text="Attendance Marked", fg='#FFFFFF', bg='GREEN')
                    except:
                        self.sl1.config(text="Server Connection Error", fg='#FFFFFF', bg='RED')
    ...
    def display_on(self):
        ...
        self.sl2.config(text="Temperature Scanning...", fg='black', bg='#D5DBDB')
